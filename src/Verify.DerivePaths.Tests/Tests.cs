...
public class Tests
{
    ...
    {
        VerifierSettings.DerivePathInfo((_, _, _, _, _, _) => new(null));
        return Verify("Value");
    }
    ...
    {
        VerifierSettings.DerivePathInfo((_, _, _, _, _, _) => new(null, null, Path.GetInvalidFileNameChars().First().ToString()));
    ...
    {
        VerifierSettings.DerivePathInfo((_, _, _, _, _, _) => new(null, Path.GetInvalidFileNameChars().First().ToString()));
        return Assert.ThrowsAsync<ArgumentException>(() => Verify("Value"));
    }
    ...
    {
        VerifierSettings.DerivePathInfo((_, _, _, _, _, _) => new(Path.GetInvalidPathChars().First().ToString()));
        return Assert.ThrowsAsync<ArgumentException>(() => Verify("Value"));
    }
}