...
sealed class DateTimePatternScrubber : PatternScrubber
{
    ...
    public DateTimePatternScrubber(
        ...
        Culture? culture)
    {
        ...
        {
            // Some calendars (e.g. UmAlQuraCalendar) can't represent DateTime.MaxValue.
        ...
        [NotNullWhen(true)] out string? replacement)
    {
        ...
        for (var len = maxLen; len >= min; len--)
        {
            ...
            if (DateTime.TryParseExact(slice, fmt, culture, DateTimeStyles.None, out var date))
            {
                matchLength = len;
