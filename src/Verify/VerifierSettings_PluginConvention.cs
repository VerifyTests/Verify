...
public static partial class VerifierSettings
{
    ...
    /// Returns <c>null</c> when the manifest cannot be found or read, in which case no filtering should be applied.
    /// </summary>
    /// <remarks>
    ...
    internal static HashSet<string>? GetReferencedAssemblyFiles(Assembly testAssembly)
    {
        ...
        try
        {
            var referenced = ReadReferencedFileNames(File.ReadAllText(depsFile));
            // An empty set would filter out every plugin, so treat it as "unable to determine".
            return referenced.Count == 0 ? null : referenced;
        }
        catch
        {
            // If the deps.json cannot be parsed, fall back to loading all plugins.
            return null;
        }
    }
