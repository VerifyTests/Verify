...
All six fixed 2026-08-16. The inline Append fix is in the DiffEngine repo (`InlinePatcher.FindCalls` now offers above-floor lines as a last resort); the rest are in this repo. Verify.Tests (1297), StaticSettingsTests (154), Verify.NUnit.Tests (233), and DiffEngine InlinePatcherTests (95) all pass.

- [x] **Inline Append anchoring breaks for same-file helpers** — regression from `31b1a427d`.
  `Verify/Verifier/InnerVerifier_Inner.cs:194` passes the framework-resolved *test method* name as `MemberName`, while file/line come from the `CallerFilePath`/`CallerLineNumber` of the actual `Verify` call. When the test calls `Verify` via a helper declared **above** the test method in the same file, the DiffEngine patcher (`DiffEngine/Inline/InlinePatcher.cs` `FindCalls`) floors its search at the member declaration line, making the real call site unreachable: accept fails forever ("Could not find a Verify call near line N"), or `.Snapshot(...)` is appended to a wrong `Verify`-prefixed call below the floor. The "only ever narrows" comment holds for the Set path (real `CallerMemberName`) but not this Append path. Scope: global-switch Append mode only.
